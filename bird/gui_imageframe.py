...
class ImageFrame():
    ...
    def __init__(self, placeholder, path, roi_size):
        ...
        self.__corners = []  # corners (max 12)
        self.__corner_coors = []  # coordinates
        ...
        self.__canvas.bind('<ButtonPress-3>', self.__move_from)  # remember canvas position
        self.__canvas.bind('<B3-Motion>', self.__move_to)  # move canvas to the new position
    ...
    def __click(self, event):
        ...
        if bbox[0] <= x < bbox[2] and bbox[1] <= y < bbox[3]:
            x1 = (x - bbox[0]) / self.__imscale
            y1 = (y - bbox[1]) / self.__imscale
            print("click at: (%f, %f)" % (x1, y1))
            self.__corner_coors.append([x1, y1])
            radius = 5.0 * self.__imscale

            oval = self.__canvas.create_oval(x - radius, y - radius, x + radius, y + radius, fill='red')
            self.__corners.append(oval)
            # print('len(self.__corners) = %d' % len(self.__corners))
    ...
    def __motion(self, event):
        ...
        # bbox = self.__canvas.coords(self.__container)  # get image area
        # if bbox[0] <= x < bbox[2] and bbox[1] <= y < bbox[3]:
        #     print("mouse position: (%f, %f)" % ((x - bbox[0]) / self.__imscale, (y - bbox[1]) / self.__imscale))
    ...
    def __wheel(self, event):
        ...
        if event.num == 5 or event.delta == -120:  # scroll down
            if int(self.__min_side * self.__imscale) < 400: return  # image is less than 30 pixels
        ...
        if event.num == 4 or event.delta == 120:  # scroll up
            ...
            # if int(self.__roi_size[0] * self.__imscale) >= 50: return
            self.__imscale *= self.__delta
            scale          *= self.__delta
    ...
    def __keystroke(self, event):
        ...
        if event.state - self.__previous_state == 4:  # means that the Control key is pressed
            ...
            elif event.keycode in [83, 40, 98, 107]:  # scroll down, keys 's' or 'Down'
                self.__scroll_y('scroll',  1, 'unit', event=event)
            elif event.keycode in [27]:  # escape to delete last annotation
                if len(self.__corners) == 0:
                    return
                # print('escape!')
                self.__canvas.delete(self.__corners[len(self.__corners) - 1])
                self.__corners.pop()
                self.__corner_coors.pop()
            elif event.keycode in [13]:  # enter to save the results
                # print('==========================')
                with open(self.path + '.coors', 'w') as out:
                    out.write(json.dumps(self.__corner_coors, indent=4))
                print('Results successfully save to %s.coors!' % self.path)
                # print(json.dumps(self.__corner_coors, indent=4))
                # print('==========================')
